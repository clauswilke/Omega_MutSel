...
Figure~\ref{nyp_bias_r2} shows the resulting relationships between $dN/dS$ and $\omega$ for each set of mutation rates (NP, yeast and polio), across M0 model codon frequency parameterizations (full regression plots are shown in Figure S1). Figure~\ref{nyp_bias_r2}A displays the estimator bias, or the systematic discrepency between the true $dN/dS$ value and the $\omega$ MLEs, and Figure~\ref{nyp_bias_r2}B displays $r^2$ values between $dN/dS$ and $\omega$. Note that the precise bias and $r^2$ values are shown in Table S1 and S2, respectively. Overall, Figure~\ref{nyp_bias_r2} reveals that $\omega$ estimates are systematically biased downwards as mutation rates become increasingly asymmetric. This trend generally holds for all 5 codon frequency parameterizations; $\omega$ inferences on alignments with NP mutation rates have the least amount of bias, followed by yeast and finally polio mutation rates, and a parallel trend exists for the correlation strengths. 

Strikingly, even though Ftrue uses the exact values intended for these codon frequency parameters, it does not dramatically outperform the other frequency parameterizations. Instead, F\xline equal generally yields the least amount of bias, in particular for polio mutation rates.  That the F61, F3x4, and CF3x4 estimators exhibit relatively strong negative biases accompanied by high $r^2$ values reveal that these estimators produce systematic $\omega$ underestimations. Taken together, these results demonstrate that the codon frequency parameters in the M0 model cannot adequately account for nucleotide compositional bias, as generated by forces other than amino-acid level selection. Therefore, future efforts in developing $\omega$-based models should investigate the utility of novel parameters which better account for asymmetry in the mutational process.
...
\noindent Table S1. Estimator bias between $\omega$ MLEs and the expected, true $dN/dS$ values, for all mutation rates and M0 codon frequency parameterizations examined. Negative bias values indicate that $\omega$ MLEs are, on average, lower than $dN/dS$. All biases are statistically significant, with all $p < 10^{-12}$.
\begin{table}[htbp]

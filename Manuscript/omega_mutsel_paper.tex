...
\noindent Table S3. Mean $\Delta$AIC for datasets simulated with NP, Yeast, or Polio mutation rates..
\begin{table}[htbp]
	\begin{tabular}{l c c c}
		...
		\noalign{\smallskip}\hline\noalign{\smallskip} 
	\end{tabular}
	Note that all models have 3 free parameters. All values in table are the mean dAIC with F61 as the reference. The ranking shown in table is the order for NP and Yeast, but note that polio is different. Even so, F61 provides the best model fit, although it certainly does not infer the most accurate $\omega$ values.
\end{table}

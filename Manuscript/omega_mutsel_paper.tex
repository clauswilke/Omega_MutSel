...
In particular, two general frameworks have been propsed to deal with underlying nucleotide and/or codon biases: target codon frequencies and target nucleotide frequencies. The first is what was proposed in GY, and the latter flavor in MG. As these parameterizations are known to influence, 
As different M0 model parameterizations are known to yield distinct $\omega$ estimates \cite{KosakovskyPondMuse2005,ZhangYu2006,Yang2006}, we inferred $\omega$ using a variety of diffferent frequency estimators. First, estimators which use target codon frequencies, and second those which use target nucleotide frequencies. 

the commonly-used frequency estimators F61 \cite{GoldmanYang1994}, F3x4 \cite{GoldmanYang1994}, CF3x4 \cite{KosakovskyPond2010}, and finally F1x4 \cite{MuseGaut1994}. F61 determines codon frequency parameters $\pi_i$ directly from the alignment's empirical codon frequencies, F1x4 estimate $\pi_i$ values using empirical nucleotide frequencies, and both F3x4 and CF3x4 estimate $\pi_i$ using the empirical nucleotide frequencies at each codon position. 
...
To derive the Fnuc model parameterization, we define $n_{ij}$ as the target nucleotide in a substitution from codon $i$ to codon $j$, and $\pi_x$ as the equilibrium frequency of nucleotide $x$. We begin with a modified version of the MG94 \cite{MuseGaut1994} instantaneous rate matrix 
...
\begin{table}[htbp]
	...
	\begin{tabular}{l c c c}
		...
		Fnuc1 & -13325.5 & -10042.0 & -5147.6 \\ 
		F1x4 & -13524.5 & -13658.5 & -15468.3 \\ 
		Fnuc3 & -14401.3 & -12851.6 & -8624.9 \\ 
...
\noindent Table S1. Estimator bias between $\omega$ MLEs and the expected, true $dN/dS$ values, for all mutation rates and M0 codon frequency parameterizations examined. Negative bias values indicate that $\omega$ MLEs are, on average, lower than $dN/dS$. All biases are statistically significant, with all $P < 2\times10^{-16}$ except for the estimator bias associated with Yeast mutation rates for Fnuc3, where $P = 5.4\times10^{-5}$.
\begin{table}[htbp]
	\begin{tabular}{c c c c c c c}
		\hline\noalign{\smallskip}
		Mutation rate & Fnuc1 & F1x4 & Fnuc3 & CF3x4 & F3x4 & F61 \\
		\hline\noalign{\smallskip}
		NP & -0.014 & -0.020 & -0.007 & -0.009 & 0.019 & -0.007 \\ 
		Yeast & 0.025 & 0.007 & -0.063 & -0.084 & -0.068 & -0.076 \\ 
		Polio & -0.049 & -0.103 & -0.088 & -0.148 & -0.136 & -0.161 \\ 
		\noalign{\smallskip}\hline\noalign{\smallskip}
	\end{tabular}
\end{table}	
...
\begin{table}[htbp]
	\begin{tabular}{c c c c c c c}
		...
		Mutation rate & Fnuc1 & F1x4 & Fnuc3 & CF3x4 & F3x4 & F61 \\
		\hline\noalign{\smallskip}
		NP & 0.988 & 0.989 & 0.985 & 0.986 & 0.902 & 0.977 \\ 
		Yeast & 0.943 & 0.917 & 0.905 & 0.897 & 0.889 & 0.864 \\ 
		Polio & 0.842 & 0.811 & 0.777 & 0.754 & 0.752 & 0.781 \\ 
		\noalign{\smallskip}\hline\noalign{\smallskip}
	\end{tabular}
\end{table}	
